...
### States Plot
- Note: plot_usmap() expects a two column dataframe as input (see ?plot_usmap for geo input options)
- Default labeling is a little messy. Custom labels can be added on top of the base usmap
- Binning continuous variables tends to make values between states easier to read

```{r stateplot1, dev = "png"}
...
- usmap::plot_usmap() is great for fast metric exploration by state
- Customizing and cleaning up default visualizations can take some leg work
